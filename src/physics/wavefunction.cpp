...
amplitude(std::vector<double>(basis_.grid_size()))
{
    ...
    */
    ...
    IO::msg::construct<std::string>("electron state", {{std::string(), state}});
...
auto print_states(std::vector<Electron> states) -> void
{
    ...
    {        
        printf(" %7s %13.5f %13.5f %13.5f %13.5f\n",
            psi.state.c_str(), psi.energy, PhysicalConstants::Eh_to_eV(psi.energy), psi.r1_moment, psi.r2_moment);
    }
}